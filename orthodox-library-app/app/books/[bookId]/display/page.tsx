...
export default function DisplayPage() {
  ...
  useEffect(() => {
    Promise.all([fetchBook(bookId), fetchBookStructure(bookId)])
      .then(([b, struct]) => {
        ...
        if (struct.length > 0) setSection(struct[0].sectionId);
      })
  ...
  useEffect(() => {
    if (!bookId) return;
    fetchBlocks(bookId, section || undefined).then(bl => {
      setBlocks(bl);
      setIdx(0);
    });
  }, [bookId, section]);
  ...
  if (loading) return (
    <div className="fixed inset-0 bg-gray-900 flex items-center justify-center text-white">Loading...</div>
  );

  if (!book) return (
    <div className="fixed inset-0 bg-gray-900 flex flex-col items-center justify-center text-white">
      <p className="text-2xl mb-4">Book not found</p>
      <Link href="/books" className="text-blue-400 hover:text-blue-300">← Books</Link>
    </div>
  );
  ...
  if (blocks.length === 0) return (
    <div className="fixed inset-0 bg-gray-900 flex flex-col items-center justify-center text-white">
      ...
      {section && <p className="text-gray-400 mb-2">Section: {section}</p>}
    ...
    <>
      ...
      <div className={`fixed inset-0 ${bg} flex items-center justify-center p-8 pb-24`}>
        <div className="text-center max-w-4xl">
          {block?.role && (
            <p className="text-white text-lg font-semibold mb-4 opacity-60 uppercase tracking-widest">{block.role}</p>
          )}
      ...
      {showSectionPicker && structure.length > 0 && (
        <div className="fixed inset-0 bg-black/70 z-20 flex items-center justify-center px-4"
          onClick={() => setShowSectionPicker(false)}>
          <div className="bg-gray-800 rounded-xl p-4 max-w-sm w-full max-h-80 overflow-y-auto"
            onClick={e => e.stopPropagation()}>
            <p className="text-white font-semibold mb-3 text-sm">Choose Section</p>
            <button
              onClick={() => { setSection(''); setShowSectionPicker(false); }}
              className={`w-full text-left px-3 py-2 rounded-lg text-sm mb-1 ${
                !section ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700'
              }`}>
              All Sections
            </button>
            {structure.map(s => (
              <button key={s.sectionId}
                onClick={() => { setSection(s.sectionId); setShowSectionPicker(false); }}
                className={`w-full text-left px-3 py-2 rounded-lg text-sm mb-1 ${
                  section === s.sectionId ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-700'
                }`}>
                {s.sectionId}
              </button>
            ))}
      ...
      {/* Control bar */}
      <div className="fixed bottom-0 left-0 right-0 bg-black/80 text-white px-4 py-3 flex items-center justify-between gap-4 flex-wrap z-10">

        <div className="flex items-center gap-2">
          <button onClick={() => setIdx(i => Math.max(i-1,0))} disabled={idx===0}
            className="px-3 py-1.5 bg-gray-700 rounded hover:bg-gray-600 disabled:opacity-30 text-sm">← Prev</button>
          <span className="text-sm text-gray-400">{idx+1}/{blocks.length}</span>
          <button onClick={() => setIdx(i => Math.min(i+1,blocks.length-1))} disabled={idx===blocks.length-1}
            className="px-3 py-1.5 bg-gray-700 rounded hover:bg-gray-600 disabled:opacity-30 text-sm">Next →</button>
        </div>
        ...
        <div className="flex gap-1">
          {langs.map(lang => (
            <button key={lang} onClick={() => setLanguage(lang)}
              className={`px-3 py-1 text-sm rounded ${language===lang?'bg-blue-600':'bg-gray-700 hover:bg-gray-600'}`}>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1">
            ...
            <input type="range" min={2} max={6} value={fontSize}
              onChange={e => setFontSize(+e.target.value)} className="w-16 accent-blue-500" />
            <span className="text-sm text-gray-400">A</span>
          </div>
          <button onClick={() => setAutoScroll(a => !a)}
            className={`px-3 py-1 rounded text-sm ${autoScroll?'bg-green-600':'bg-gray-700 hover:bg-gray-600'}`}>
