...
export default function StudyPage() {
  ...
  useEffect(() => {
    Promise.all([fetchBook(bookId), fetchBlocks(bookId)])
      .then(([b, bl]) => { setBook(b); setBlocks(bl); if (b?.languages?.length) setCols(b.languages.slice(0,3)); })
  ...
  return (
    <div className="w-full">
      ...
      {blocks.length === 0 ? (
        <div className="text-center py-16 text-gray-400"><p className="text-4xl mb-3">🔀</p><p>No content yet.</p></div>
      ) : stacked ? (
        ...
        <div className="p-4">
          <div className="grid gap-4 mb-4 sticky top-14 bg-white py-2" style={{ gridTemplateColumns: `repeat(${cols.length}, 1fr)` }}>
            {cols.map((lang, i) => (
              <div key={lang} className={`font-bold text-center pb-2 border-b-2 ${colColors[i]?.split(' ')[0]}`}>{LANG_LABELS[lang] || lang}</div>
            ))}
          </div>
