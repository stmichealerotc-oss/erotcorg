        ...
        <div className="flex gap-2 flex-wrap">
          {(book.languages || ['en']).map(lang => (
            <button key={lang} onClick={() => setLanguage(lang)}
              className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${language === lang ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
      ...
      {blocks.length === 0 ? (
        ...
        <div className="space-y-3">
          {blocks.map(block => {
            ...
            return (
              <div key={block._id} className={`p-4 rounded-lg bg-white shadow-sm ${ROLE_COLOR[block.role] || ''} ${block.isRubric ? 'bg-red-50 italic' : ''}`}>
                {block.role && <p className="text-xs font-semibold text-gray-400 uppercase mb-1">{block.role}</p>}
